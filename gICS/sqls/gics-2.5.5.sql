...
begin
	...
	INSERT INTO stat_value (stat_value_id,stat_attr,stat_value) values (@id, 'revocations',(SELECT count(*) as num_revocations FROM (select * from gics.signed_policy where status=5) as revocations));
...
create user 'gics_user'@'%' identified by 'gics_2016';
grant all on gics.* to 'gics_user'@'%' identified by 'gics_2016';
