...
CREATE TABLE `person_preprocessed` (
  ...
  CONSTRAINT `FK_person_preprocessed-person_group` FOREIGN KEY (`person_group_id`) REFERENCES `person` (`personGroup_id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE USER 'epix_user'@'%' IDENTIFIED BY 'epix_2016';
GRANT ALL PRIVILEGES ON epix.* TO 'epix_user'@'%' IDENTIFIED BY 'epix_2016';
